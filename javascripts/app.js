$(document).ready(function() {
    ...
    var lock = new Auth0Lock(
      'DqvUsrTr5HKRkRT5YiC3TsMU8olil5r2',
      'scaleworks.auth0.com'
    );
    ...
    function goToHomepage(state, token) {
      ...
      console.log(profile);
      $('<img src="'+ profile.picture +'">').load(function() {
        $(this).width(100).height(100).appendTo('#profile-image');
      });
      $('.name').text(profile.name);
      $('.uid').text(profile.sub);
      if (state) {
        $('.url').show();
        $('.url span').text(state);
      }
